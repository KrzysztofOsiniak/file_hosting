...
func subtestDeleteFile(t *testing.T) {
	...
	request := &http.Request{Method: "DELETE", URL: &url.URL{Scheme: "https", Host: serverHost, Path: "/api/file/" + strconv.Itoa(testUser.FileID)}, Proto: "2.0", Header: header}
	...
	if res.StatusCode != 200 {
		t.Fatal("Server did not reply with 200 on DELETE file")
	}
}
