...
func subtestPatchFolderName(t *testing.T) {
	...
	folder := folderNamePatch{
		Name: "changedFolderName",
		ID:   testUser.FolderID,
	}
	...
	request := &http.Request{Method: "PATCH", URL: &url.URL{Scheme: "https", Host: serverHost, Path: "/api/file/folder/name"}, Proto: "2.0", Header: header, Body: body}
	...
	if res.StatusCode != 200 {
		t.Fatal("Server did not reply with 200 on PATCH folder name")
	}
}
