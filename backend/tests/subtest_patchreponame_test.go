...
func subtestPatchRepositoryName(t *testing.T) {
	...
	request := &http.Request{Method: "PATCH", URL: &url.URL{Scheme: "https", Host: serverHost, Path: "/api/repository/name"}, Proto: "2.0", Header: header, Body: body}
	...
	if res.StatusCode != 200 {
		t.Fatal("Server did not reply with 200 on PATCH repository name")
	}
}
